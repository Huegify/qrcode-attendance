...
function loadNavPage(str,divid, pageid) 
{ 	
	if(str!='#'){
		...
		$.ajax({ 
			...
			error: function(x, t, m) {
				$.blockUI({ message:'Page not found'});
...
function myLoadModal(url, div) {
  getpage(url, div);
  $("#defaultModal").modal("show");
}

function regenerateCORS(){
	var ip = window.location.host, url;
	if (ip == 'localhost' || ip == '::1') {
		// url = document.location.hostname + path + file;

		url = window.location.protocol + "//" + window.location.host + "/lfs/";
	} else {
	...
	$.ajax({
		type: "POST",
		url: 'helper',
		data:{op:'Model.regenerateCORS', 'regenerate':true},
		success: function(data){
			// console.log(data);
			$('#att-csrf-token-label').attr('value',data);
		},
		error: function(){
			console.log('failed');
		}
...
// var f = document.querySelectorAll('[data-bs-toggle="collapse"]')
// f.forEach((dd)=>{
...
//     var lastActive = $(this).closest(".sidebar-item").children(".collapse");
//     lastActive.removeClass("collapse");
//     lastActive.addClass("collapsed");
//     lastActive.next('ul > sidebar-dropdown').collapse('show');
...
$(document).on('click', '.expand-nav', function(e){
 $(this).hide();
  $('.expand-nav').removeClass('expand-nav');
  $('.expand-nav').addClass('remove-nav');
  $('.navbar-collapse').toggle();
  // $('.navbar-collapse').hide().animate('slide');

})

$(document).on('click', '.remove-nav', function(e){
  // alert();
  $('.expand-nav').removeClass('remove-nav');
  $('.expand-nav').addClass('expand-nav');
  $('.navbar-collapse').show().animate('slide');
...
function upload_excel_content(form_id, url) {
  ...
  $.blockUI({
    message:
      '<img src="images/loading.gif" alt=""/>&nbsp;&nbsp;processing request please wait . . .',
  });
  formurl = url + ".php";
    ...
    success: function (response) {
      ...
      if (response) {
        if (response["code"][0] == 0) {
          ...
          $("#table").html(
            "<h3 style='color:red'>" + response["message"][0] + "</h3>"
          );
        }
      }
    },
...
function get_content(response) {
  var tr =
    '<form id="myform"><table class="table"><input type="hidden" name="upload_id" value="' +
    response["data"][0].upload_id +
    '" />';
  tr +=
    "<tr><th>Vehicle Name</th><th>Chasis No</th><th>Renewal State</th><th>Date of expiration</th></tr>";
  for (var i = 0; i < response["data"].length; i++) {
    ...
    tr +=
      '<td nowrap="nowrap">' +
      '<input type="date" name="date[]" value="" ><input type="hidden" name="chasis_no[]" value="' +
      response["data"][i].chasis_no +
      '" ><input type="hidden" name="payment_item[]" value="' +
      response["data"][i].payment_item +
  ...
  console.log("hdhd");
  tr +=
    '<tbody></table> <br/><br/> <input type="button" onclick="update_autoreg()" class="btn btn-block btn-success" name="send" value="Submit"></form>';
  setTimeout(function () {
    $("#table").html(tr);
    console.log(tr);
    $("#state_warning").show();
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data == 0) {
        ...
        alert(
          "Some or all records were not saved.. please check your excel data and try again"
        );
      }
    },
    error: function (data) {
...
function checkboxVal(obj) {
  if (obj.checked) {
    ...
    $("#subbtn")
      .attr("value", "Read the Terms Before Proceeding!")
      .show("slow");
  }
}
function cal_amt() {
...
function checklogin(formurl, formurlconvert) {
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      $("#error_label_login")
        .html(
          "<div class=' btn btn-success btn-block '>logging you in ...</div>"
        )
        .show();
      if (msg == "") {
        $("#error_label_login")
          .html(
            "<div class=' btn btn-danger2 btn-block '>Please enter a valid Username and Password</div>"
          )
          .show();
      } else if (msg == "0") {
        $("#error_label_login")
          .html(
            "<div class='btn btn-danger2 btn-block '>Invalid username or password</div>"
          )
          .show();
      } else if (msg == "1") {
        //	alert(msg);
        ...
        $("#error_label_login")
          .html("Your user profile has been disabled")
          .show();
      } else if (msg == "3") {
        $("#error_label_login")
          .html(
            "Your user profile has been locked, please contact Administrator"
          )
          .show();
      } else if (msg == "4") {
        $("#error_label_login")
          .html("You are not allowed to login on Sunday")
          .show();
      } else if (msg == "5") {
        $("#error_label_login")
          .html("You are not allowed to login on Monday")
          .show();
      } else if (msg == "6") {
        $("#error_label_login")
          .html("You are not allowed to login on Tuesday")
          .show();
      } else if (msg == "7") {
        $("#error_label_login")
          .html("You are not allowed to login on Wednesday")
          .show();
      } else if (msg == "8") {
        $("#error_label_login")
          .html("You are not allowed to login on Thursday")
          .show();
      } else if (msg == "9") {
        $("#error_label_login")
          .html("You are not allowed to login on Friday")
          .show();
      } else if (msg == "10") {
        $("#error_label_login")
          .html("You are not allowed to login on Saturday")
          .show();
      } else if (msg == "11") {
        $("#error_label_login")
          .html(
            "You are not allowed to login at this time <br> The time is not within the working hours"
          )
          .show();
      } else if (msg == "12") {
        $("#error_label_login")
          .html("Your profile has been Locked, please contact Administrator")
          .show();
      } else if (msg == "13") {
        $("#error_label_login")
          .html(
            "Your password has expired, <br><a href='change_password_exp.php?id=" +
              data +
        ...
        $("#error_label_login")
          .html(
            "You are required to change your password, <br><a href='change_password_logon.php?id=" +
              data +
              "'> click here to change password </a>"
          )
          .show();
        ...
        $("#error_label_login")
          .html(
            "You did not logout the last time you logged in. Pls Login again .. "
          )
          .show();
      } else {
        $("#error_label_login")
          .html("Invalid username or password" + msg)
          .show();
      }
    },
  });
  ...
  if (data != "error") {
    ...
    $.ajax({
      ...
      success: function (msg) {
        ...
        if (
          msg ==
          '<div class="alert alert-success">The User password has been successfully changed </div>'
        ) {
        ...
        if (page == "save_merchant" && ro == "003") {
          if (
            msg ==
            "User detail already exist, please enter a different username"
          ) {
            $("#display_message").html(
              '<div class="btn alert-danger">User detail already exist, please enter a different username</div>'
            );
          } else {
        ...
        if (page == "save_head_office") {
          ...
          if (
            msg ==
            '<div class="btn alert-success">User detail has been successfully saved</div>'
          ) {
            setTimeout(function () {
              getpage("head_office_list.php", "page");
            }, 2000);
          }
        } else {
...
function check_textvalues(formElement) {
  ...
  if (triminput(formElement.val()) == "") {
    $("#display_message").html(
      '<div class="btn btn-block btn-danger2 ">please enter value for :' +
...
function check_numbers(formElement) {
  if (triminput(formElement.val()) == "") {
    $("#display_message").html(
      '<div class="btn btn-block btn-danger2">please enter number for : ' +
        formElement.attr("title") +
        "</div>"
    );
    $("#display_message").show("fast");
...
function check_email(formElement) {
  ...
  if (formElement.val().search(emailRegEx) == -1) {
    ...
    $("#display_message").html(
      '<div class="btn btn-block btn-danger2">please enter valid email for : ' +
        formElement.attr("title") +
        "</div>"
    );
    $("#display_message").show("fast");
...
function check_captcha(formElement) {
  ...
  if (captcha != captcha_sess) {
    $("#display_message").html(
      '<div class="btn btn-block btn-danger2">Please Enter Figures Displayed in ImageCaptcha into textbox above</div>'
    );
    $("#display_message").show("fast");
    formElement.focus();
    $("#display_message").click();
...
function check_password_aplhanumeric(formElement) {
  ...
  if (
    ...
    $("#display_message").html(
      '<div class="btn btn-block btn-danger2">please enter alphanumeric as password</div>'
    );
    $("#display_message").show("fast");
    //alert('failed');
    formElement.focus();
...
function callpagepost(str, divid) {
  ...
  if (data != "error") {
    ...
    $.blockUI({
      message:
        '<img src="images/loading.gif" alt=""/><br />processing request please wait . . .',
    });
    //});
...
function log() {
  $.ajax({
    type: "POST",
    url: "admin/helper.php?op=audit_report",
    data: "",
    //timeout:1000,
    error: function (x, t, m) {
      // $.unblockUI();
...
function printDiv(seldiv) {
  ...
  newWin.document.write(
    '<html><link rel="stylesheet" type="text/css" href="css/light.css"><body><div class="block-fluid clearfix" style="padding-top:10px;">' +
...
function printReceipt(seldiv) {
  ...
  /*newWin.document.open();*/
  newWin.document.write(
    '<html><link rel="stylesheet" type="text/css" href="css/light.css"><body onload="window.print()">' +
      divToPrint.innerHTML +
      "</body></html>"
  );
  newWin.document.close();
...
function getmerchantcat(str, divid) {
  ...
  if (str != "#") {
    $("#" + divid)
      .html(
        '<div class="loading"><center><img src="images/285.gif" alt=""/><br>loading please wait . . .</center></div>'
      )
  ...
  if (obj.checked) {
    ...
    for (var i = 0; i < frm.elements.length; i++) {
      if (
        ...
        frm.elements[i].name.search(obj.value) != -1
      ) {
...
function updateUser(strr, str) {
  ...
  if (strr == "101" && str != "") {
    $("#resu").html(
      "Welcome " + str + ' <font color="#009900">You are now logged in</font>'
    );
    $(".rerun").show();
    $("#resu_recharge").show();
    $("#resu_balance").show();
...
function validateCustomer(errlb, lgn, formurl, mid) {
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      if (retstr[0] == "") {
        $("#" + errlb)
          .html("Please enter a valid Username and Password")
          .show();
      } else if (retstr[0] == "0") {
        $("#" + errlb)
        ...
        $("#resu")
          .html(
            "Welcome " +
              custName +
              ' <font color="#009900"><br />You are now logged in</font>'
          )
          .css("font-size", "10px");
        $(".rerun").show();
        $("#resu_recharge").show();
        ...
        $("#" + errlb)
          .html("Your user profile has been locked")
          .show();
      } else {
        $("#" + errlb)
          .html("Invalid username or password")
...
function makePayment(str) {
  if (
    ...
    var mygtw =
      $("#amount").val() + "::" + $("#tType").val() + "::" + $("#trDesc").val();
    $.ajax({
      type: "POST",
      url: "admin/helper.php",
      ...
      success: function (msg) {
        ...
        if (myRes[0] == 0) {
          $("#reponse_message")
            .html("No Payment Gateway Selected Yet ! !! !!! ")
            .css("color", "#F00");
    ...
    $("#reponse_message")
      .html(
        "No Payment Gateway Selected Yet ! !! !!! Select a Payment Gateway and try again"
      )
      .css("color", "#F00");
  ...
  $.ajax({
    ...
    success: function (msg) {
      if (msg == 1) {
        ...
        $("#err_messg").html(
          "No Item Selected : Please Select Item and try again"
        );
      }
    },
  });
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      if (msgg[0] == "1") {
        $("#balance").html(msgg[1]);
        $("#" + str)
          .html("Your transaction has been successfully completed.")
          .css({ color: "#999" });
      } else if (msgg[0] == "2") {
        $("#" + str)
          .html("ERROR ! !! !!! Invalid transaction details")
          .css({ color: "#999" });
      } else if (msgg[0] == "3") {
        $("#" + str)
          .html("ERROR ! !! !!! System error has occur")
          .css({ color: "#999" });
      } else if (msgg[0] == "4") {
        $("#" + str)
          .html(
            "ERROR ! !! !!! You have insufficient balance to carry out this transaction"
          )
          .css({ color: "#999" });
      } else if (msgg[0] == "5") {
        $("#" + str)
          .html("ERROR ! !! !!! Un-Authorized transaction")
          .css({ color: "#999" });
      }
    },
  });
...
function calldownload() {
  ...
  //alert(data);
  window.open(
    "download.php?sql=" + escape(data) + "&filename=" + data2,
    "mydownload",
    "status=0,toolbar=0"
  );
}
...
function getstate(country) {
  $("#state_div").html(
    '<img src="images/loading.gif" alt=""/>&nbsp;&nbsp;Loading states please wait . . .'
  );
...
function getLga(str) {
  ...
  $.blockUI({
    message:
      '<h5><font color="#FF0000"> Please wait, loading page LGAs ...</font></h5>',
    css: {
...
function addCoordChurch() {
  ...
  if (ch_name != "") {
    ...
    alert(
      " Church Name is Empty ! Church Cannot be set as Coordinating church"
    );
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      var chart = new CanvasJS.Chart("all_sum_com", {
        ...
        title: {
          text:
            " Last Four Attendance Review For " +
            $("#serv_id option:selected").text(),
          fontSize: 14,
        },
        dataPointWidth: 25,
        axisY: {
...
function selectvalueoptionSubject(value, operation, sname, $sname2) {
  $.blockUI({
    message:
      '<h5><font color="#FF0000"> Please wait, loading page ...</font></h5>',
    css: {
      border: "3px solid #a00",
      padding: "5px",
      "text-align": "left",
...
function importExcel() {
  ...
  $.blockUI({
    message:
      '<img src="images/loading.gif" alt=""/><br />Extracting File to Database . . .',
  });
  $.ajax({
    type: "POST",
    url: "admin/helper.php",
...
var table_select, table;
function datatable_check(op, data) {
  table_select = $("#datatables").DataTable({
    responsive: true,
    processing: true,
    columnDefs: [
      {
        ...
        render: function (data, type, full, meta) {
          return (
            '<input type="checkbox" class="checkid" name="id[]" id="checkid" value="' +
            $("<div/>").text(full[1]).html() +
            '">'
          );
        },
        select: {
    ...
    ajax: {
      url: "helper?att-csrf-token-label="+$("#att-csrf-token-label").val(),
      type: "POST",
      data: function (d, l) {
        for (var i = 0, l1 = data.length; i < l1; i++) {
          d[data[i][0]] = $("#" + data[i][1]).val();
        }
        d.op = op;
        d.li = Math.random();
      },
...
function datatable_check_filter() {
  table_select.draw();
}