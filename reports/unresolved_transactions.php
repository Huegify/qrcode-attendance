...
<div class="card">
    <div class="card-header">
        <h5 class="card-title">Unresolved Transactions</h5>
        <h6 class="card-subtitle text-muted">The report contains all the unresolved payments.</h6>
    </div>
    <div class="card-body">
        <!-- <div class="row">
            <div class="col-sm-4">
...
<script>
    $("select").select2();
    var table, op = "Transaction.unresolvedTransactions";
    ...
    $(document).on('click', '.print-receipt', function(e) {
        ...
        $.ajax({
            type: 'post',
            url: 'helper',
            data: {op: 'Transaction.generateReceipt', id: id },
            dataType: 'json',
            beforeSend: function() {
                $.blockUI({
                    message: 'Generating transaction details...'
                });
                setTimeout(function() {
                    $.unblockUI();
                }, 2000);
            ...
            success: function(data) {
                if (data.response_code == 0) {
                    setTimeout(function() {
                        window.open('receipts/' + data.response_message);
                    }, 2000);
