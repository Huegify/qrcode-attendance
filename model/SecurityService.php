 ...
 * Though not mandatory, you are requested to attribute ATTD URL in your code or website.
 * 
 * ATTD ==> NNPC Retails Fuel App
 */

namespace ATTD\SecurityService;
...
class securityService
{
    ...
    private $formTokenLabel = 'att-csrf-token-label';

    private $sessionTokenLabel = 'LFS_CSRF_TOKEN_SESS_IDX';
    ...
    public function insertHiddenToken()
    {
        ...
        echo "<!--\n--><input type=\"hidden\"" . " name=\"" . $this->xssafe($this->formTokenLabel) . "\"" . " id=\"" . $this->xssafe($this->formTokenLabel) . "\"" . "  value=\"" . $this->xssafe($csrfToken) . "\"" . " />";
    }
            ...
            if (!empty($this->post)) {
                ...
                if (!$isAntiCSRF) {
                    ...
                    return json_encode(
                        array(
                            'valid' => false,
                            'response_message' => 'Security Alert: Unable to process your request.',
